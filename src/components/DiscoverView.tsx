...
import { Search, Star, MessageSquare, Plus, Sparkles, Heart, Users, ShieldCheck, MapPin } from 'lucide-react';
...
export const DiscoverView: React.FC<DiscoverViewProps> = ({
  ...
  return (
    ...
    >
      {/* Hero Header Banner with Breathing Neon Glow */}
      <motion.div
        initial={{ opacity: 0, scale: 0.98 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.5, ease: [0.25, 1, 0.5, 1] }}
        className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#0a0a0a] via-[#111111] to-[#0a0a0a] border border-white/10 p-6 sm:p-10 shadow-2xl"
      >
        {/* Breathing Neon Radial Glow Background */}
        <motion.div
          animate={{
            scale: [1, 1.2, 1],
            opacity: [0.2, 0.4, 0.2],
        ...
        <div className="relative z-10 max-w-3xl space-y-4">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-extrabold shadow-inner"
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white leading-tight">
            Yerli Türk Ajanlar & <span style={{ color: accentHex, textShadow: `0 0 25px ${accentHex}80` }}>XASİL Siber Evren</span>
          </h1>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            {onOpenGroupChat && (
              <motion.button
                ...
                className="px-6 py-2.5 rounded-full bg-gradient-to-r from-amber-500 via-rose-500 to-purple-600 text-white font-extrabold text-sm flex items-center gap-2 shadow-lg hover:shadow-rose-500/30 transition-all cursor-pointer border border-white/20 animate-pulse"
              >
            ...
            <motion.button
              whileHover={{ scale: 1.05, boxShadow: `0 0 25px ${accentHex}80` }}
              whileTap={{ scale: 0.95 }}
              onClick={onCreateNew}
              className="px-6 py-2.5 rounded-full text-black font-extrabold text-sm flex items-center gap-2 shadow-lg transition-all cursor-pointer"
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Category Tabs - Rounded Pills with Motion */}
          <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
            {CATEGORIES.map((cat) => (
              <motion.button
                ...
                className={`px-5 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all cursor-pointer relative ${
                ...
                style={selectedCategory === cat ? { backgroundColor: accentHex, boxShadow: `0 0 18px ${accentHex}60` } : {}}
              >
                {cat}
              </motion.button>
            ))}
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full bg-[#121212] text-white placeholder-gray-500 text-xs rounded-full pl-10 pr-4 py-2.5 border border-white/10 focus:border-[#00D2FF] focus:outline-none transition-all shadow-inner"
            />
          </div>
        </div>
      </div>

      {/* Cinematic Grid Cards with Card Flip & Scale-In Animations */}
      <motion.div
        layout
      ...
      >
        <AnimatePresence mode="popLayout">
          {filteredAgents.map((agent, index) => {
            const isFeatured = agent.id.includes('hakan');
            ...
            return (
              <motion.div
                ...
                className={`group bg-[#121212] rounded-3xl border ${
                  isFeatured ? 'border-[#00D2FF]/60 shadow-[0_0_25px_rgba(0,210,255,0.25)]' : 'border-white/10'
                } overflow-hidden hover:border-[#00D2FF]/50 transition-colors cursor-pointer flex flex-col justify-between relative shadow-xl hover:shadow-[0_12px_30px_rgba(0,0,0,0.8)]`}
              >
                {/* Featured Glowing Outline Ribbon */}
                {isFeatured && (
                  <div
                    className="absolute inset-0 pointer-events-none rounded-3xl border-2 border-[#00D2FF] opacity-60 animate-glow-pulse"
                ...
                <div>
                  ...
                  <div className="p-5 space-y-2.5">
                    <div>
                      <h3 className="text-lg font-bold text-white group-hover:text-[#00D2FF] transition-colors flex items-center justify-between">
                    ...
                    <div className="space-y-1.5 pt-1">
                      ...
                      <div className="w-full bg-[#050505] rounded-full h-1.5 overflow-hidden border border-white/10">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${Math.min(100, agent.bond || agent.relationshipLevel || 30)}%` }}
                ...
                <div className="px-5 pb-5 pt-2 border-t border-white/5 flex items-center justify-between">
                  ...
                  <motion.button
                    ...
                    style={{ borderColor: `${accentHex}50`, color: accentHex, backgroundColor: `${accentHex}15` }}
                  >
      ...
      {filteredAgents.length === 0 && (
        <motion.div
          ...
          animate={{ opacity: 1, scale: 1 }}
          className="text-center py-16 bg-[#121212] rounded-3xl border border-white/10 space-y-3"
        >
